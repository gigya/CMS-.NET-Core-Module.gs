...
var gigyaSitefinity = {
    ...
    redirectAfterLogin: function redirectAfterLogin(url) {
        if (gigyaSitefinity.loggedInRedirectUrl != '') {
            url = gigyaSitefinity.loggedInRedirectUrl;
        }
    ...
    applyConfig: function applyConfig() {
        ...
        gigyaSitefinity.id = window.gigyaConfig.id;
        gigyaSitefinity.loggedInRedirectUrl = window.gigyaConfig.loggedInRedirectUrl;
        gigyaSitefinity.logoutRedirectUrl = window.gigyaConfig.logoutRedirectUrl;
        gigyaSitefinity.debugMode = window.gigyaConfig.debugMode;

        if (window.gigyaConfig.logoutApiAction) {
            gigyaSitefinity.logoutApiAction = window.gigyaConfig.logoutApiAction;
        }
    ...
    showScreenSet: function showScreenSet(settings, event) {
        ...
        gigyaSitefinity.log('screen set settings', settings);
    ...
    initialized: false,
    init: function init() {
        if (gigyaSitefinity.initialized) {
            gigyaSitefinity.log('gigyaSitefinity already initialized...ignoring');
            return;
        }

        gigyaSitefinity.attachEvents(document.getElementsByClassName('gigya-login'), 'click', function (event) {
            gigyaSitefinity.showScreenSet(gigyaSitefinity.screenSetSettings.login, event);
            event.preventDefault();
            return false;
        });

        gigyaSitefinity.attachEvents(document.getElementsByClassName('gigya-logout'), 'click', function (event) {
            gigya.accounts.logout({ callback: gigyaSitefinity.onLogoutTriggeredByUser });
            event.preventDefault();
            return false;
        });

        gigyaSitefinity.attachEvents(document.getElementsByClassName('gigya-register'), 'click', function (event) {
            gigyaSitefinity.showScreenSet(gigyaSitefinity.screenSetSettings.register, event);
            event.preventDefault();
            return false;
        });

        gigyaSitefinity.attachEvents(document.getElementsByClassName('gigya-edit-profile'), 'click', function (event) {
    ...
    onGetAccountInfo: function onGetAccountInfo(eventObj) {
        ...
        if (eventObj.errorCode == 0) {
            gigyaSitefinity.login(eventObj, false, gigyaSitefinity.loginSource.GetAccountInfo);
        } else {
    ...
    onProfileUpdated: function onProfileUpdated(eventObj) {
        ...
        var data = {
            ...
            Id: gigyaSitefinity.id
        };

        qwest.post(gigyaSitefinity.baseUrl + 'editprofile', data).then(function (xhr, response) {
            if (response != null && response.status == gigyaSitefinity.responseCodes.Success) {
                gigyaSitefinity.redirectAfterEditProfile(response.redirectUrl);
            } else {
    ...
    onLogin: function onLogin(eventObj) {
        ...
        gigyaSitefinity.login(eventObj, true, gigyaSitefinity.loginSource.Login);
    },
    ...
    onLogoutTriggeredByUser: function onLogoutTriggeredByUser(response) {
        gigyaSitefinity.log('onLogout triggered by user');
        gigyaSitefinity.logout(true);
    },
    onLogout: function onLogout(eventObj) {
        gigyaSitefinity.log('onLogout', eventObj);
        gigyaSitefinity.logout(false);
    },
    logout: function logout(redirectAfterLogout) {
        var data = { Id: gigyaSitefinity.id, Force: redirectAfterLogout };

        qwest.post(gigyaSitefinity.baseUrl + gigyaSitefinity.logoutApiAction, data).then(function (xhr, response) {
            if (response != null) {
                switch (response.status) {
                    case gigyaSitefinity.responseCodes.Success:
                        if (redirectAfterLogout) {
                            gigyaSitefinity.redirectAfterLogout(response.redirectUrl);
                        }
                        break;
                }
            } else {
